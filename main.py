...
async def help_text(message):
    await message.answer("/me {имя} - Команда для того чтобы определить к какой нации относится данное имя")
    # await bot.send_message(message.chat.id, "/me {имя} - Команда для того чтобы определить к какой нации относится данное имя")
